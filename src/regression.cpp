...
nucmath::Regression::LinearRegressionResult nucmath::Regression::linear(const std::vector<std::array<double, 2>>& points)
{
    ...
    const double b_counter = (dN * sum_x_y - sum_x * sum_y);
    const double b_denominator = (dN * sum_x2 - sum_x * sum_x);
    ...
    result.a0 = (sum_y - result.a1 * sum_x) / dN;
    result.r2 = 0;
    ...
    const double R2_denominator = (dN * sum_x2 - sum_x * sum_x) * (dN * sum_y2 - sum_y * sum_y);
    if(!isEqual(R2_denominator, 0))
        result.r2 = R2_counter / R2_denominator;

    result.r2_red = 1.0 - (1.0 - result.r2) * (dN - 1.0) / (dN - 2.0);
